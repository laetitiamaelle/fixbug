...
export default function ApercuProjetPage() {
  ...
  function bugsVersMessages(liste: BugItem[]): Message[] {
    ...
    [...liste]
      ...
      .forEach((b) => {
        ...
        msgs.push({ key: `bot-${b.id}`, role: "bot", bugId: b.id, description: "", statut: b.statut, createdAt: b.createdAt });
      });
    return msgs;
  }
  ...
  // Chaque bug est un fil séparé : "nouveau" n'affiche que le brouillon en
  // cours d'envoi, la sélection d'un bug n'affiche que ses 2 messages à lui.
  const messagesAffiches =
    selection === "nouveau"
      ? messages.filter((m) => m.key.startsWith("temp-"))
      : messages.filter((m) => m.bugId === selection.id);

  useEffect(() => {
    finChatRef.current?.scrollIntoView({ behavior: "smooth", block: "end" });
  }, [messagesAffiches.length, selection]);

  // Le textarea grandit avec le contenu.
  useEffect(() => {
  ...
  async function declarerBug(contenu: string, fichiersListe: File[], messageKey?: string) {
    ...
    try {
      ...
      const bug: BugItem = await apiFetch("/bugs", { method: "POST", body: formData });

      setMessages((prev) => {
        const ancien = prev.find((m) => m.key === messageKey);
        ancien?.captures?.forEach((url) => { if (url.startsWith("blob:")) URL.revokeObjectURL(url); });

        const confirme: Message = {
          key: `bug-${bug.id}`,
          role: "user",
          bugId: bug.id,
          titre: bug.titre,
          description: bug.description,
          captures: bug.captures,
          statut: bug.statut,
          testeur: bug.testeur,
        ...
        return [...prev.map((m) => (m.key === messageKey ? confirme : m)), botMsg];
      });

      setBugs((prev) => (prev ? [...prev, bug] : [bug]));
      // On bascule sur le fil du bug qui vient d'être créé, pour voir la confirmation.
      setSelection(bug);
  ...
  function handleEnvoyer(e?: React.SyntheticEvent) {
    e?.preventDefault();
    ...
    <div className="grid h-[calc(100vh-190px)] grid-cols-1 gap-4 md:grid-cols-[240px_1fr]">
      ...
      <div className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 p-3">
          <Button onClick={handleNouveauBug} size="sm" className="w-full justify-start bg-[#12151F] hover:bg-[#12151F]/90">
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="space-y-0.5 p-2">
            {bugs === null ? (
              ...
              bugs.map((bug) => {
                const s = configStatut[bug.statut];
                const Icone = s.icone;
                const actif = selection !== "nouveau" && selection.id === bug.id;
                return (
                  <button
                    key={bug.id}
                    onClick={() => setSelection(bug)}
                  ...
                  >
                    <Icone className={`h-3.5 w-3.5 shrink-0 ${actif ? "text-slate-700" : "text-slate-400"}`} />
      ...
      <div className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          <div className="flex flex-col gap-4 p-5">
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="max-w-[80%] rounded-2xl rounded-bl-sm bg-slate-100 px-4 py-3">
                <p className="mb-1 text-xs font-medium text-slate-500">Assistant Fixbug</p>
                <p className="text-sm text-slate-700">
                  {selection === "nouveau"
                    ? "Bonjour, décrivez le bug rencontré ci-dessous et joignez des captures si besoin."
                    : "Voici le suivi de ce signalement."}
                </p>
              </div>
            </div>

            {messagesAffiches.length === 0 && selection === "nouveau" && !envoi && (
              <p className="px-1 text-center text-xs text-slate-400">
                Écrivez votre message ci-dessous pour déclarer un nouveau bug.
              </p>
            )}
                ...
                <div key={msg.key} className="flex justify-end">
                  <div className="max-w-[80%] rounded-2xl rounded-br-sm bg-[#12151F] px-4 py-3">
                    <p className="mb-1 text-xs font-medium text-white/50">
                      {msg.testeur ? `${msg.testeur.prenom} ${msg.testeur.nom}` : "Vous"}
                    </p>
        ...
        {/* Barre de saisie : n'apparaît que pour "Nouveau bug" — on ne répond pas dans un fil déjà clos */}
        {selection === "nouveau" ? (
          <form onSubmit={handleEnvoyer} className="shrink-0 p-3">
            {fichiers.length > 0 && (
          ...
          <div className="shrink-0 border-t border-slate-100 p-3 text-center">
            <button
              type="button"
              onClick={handleNouveauBug}
              className="text-sm font-medium text-slate-500 hover:text-[#12151F]"
            >
              Déclarer un autre bug →
            </button>
          </div>
        )}
      </div>
