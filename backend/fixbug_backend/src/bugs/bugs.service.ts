...
export class BugsService {
  ...
  async demanderAnalyseIA(
    bugId: number,
    developpeurId: number,
    instructionDeveloppeur: string | undefined,
    ...
    const { resumeIA, propositions } = await this.agentIaService.analyserBug(contexte, fichiers, bug.captures);
  ...
  async pousserSurGithub(bugId: number, developpeurId: number, fichiers: { chemin: string; contenu: string }[]) {
    const bug = await this.prisma.bug.findUnique({ where: { id: bugId }, include: { projet: true } });
    if (!bug) throw new NotFoundException('Bug introuvable');
    if (bug.developpeurId !== developpeurId) throw new ForbiddenException("Ce bug ne vous est pas assigné");
    if (!fichiers || fichiers.length === 0) throw new BadRequestException('Aucun fichier à pousser');

    const nomBranche = `fix/bug-${bug.id}`;
    await this.agentIaService.pousserTousLesFichiers(bug.projet.liengit, nomBranche, fichiers);
